...
const wchar_t* pWndTitle = L"MadSweets";
...
HWND InitInstance(HINSTANCE hInstance, int nCmdShow, bool isFullScreen, int iClientWidth, int iClientHeight)
{
	...
	if (isFullScreen) { // フルスクリーン
		...
		hWnd = CreateWindow(
			pClassName,				// 登録されているクラス名
			pWndTitle,				// ウインドウ名
			WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,	// ウインドウスタイル（オーバーラップウインドウを作成）
	...
	_In_ int       nCmdShow)
{
	...
	//ウインドウの幅と高さ
	int iClientWidth = WindowSetting::WINDOW_WIDTH;
	int iClientHeight = WindowSetting::WINDOW_HEIGHT;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:                // キーが押された
		...
		{
			App::GetApp()->OnKeyDown(static_cast<UINT8>(wParam));
		}
